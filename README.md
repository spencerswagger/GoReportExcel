...
编辑器路由 `/editor/:id`：左栏配置（维度排序/指标/样式规则图层/条件格式/页面设置）、中栏实时预览（样式字典 CSS、合并、条件格式 JS 模拟、虚拟滚动、缩放 50%–200%）、右栏检查器（样式解释/数据血缘/预览直改生成 override）。顶栏深墨工作台支持防抖自动保存、撤销/重做、预设主题下拉、发布、版本历史与回滚、异步导出（轮询进度 + 下载）。

界面采用「报表工坊」设计体系：深墨工作台 chrome + 暖米白纸张画布 + 铜金琥珀强调色，衬线品牌字标（Noto Serif SC）+ 等宽数字（IBM Plex Mono），所有图标/文案保持可访问性。
